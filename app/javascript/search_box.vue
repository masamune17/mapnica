    ...
    methods:{
      ...
      async outputResult(){
        ...
        for (let i = 0; i < searcheWordApi.length; i++) {
          const labelSearch = searcheWordApi[i].label.toLowerCase().indexOf(this.keyword)
          if (labelSearch !== -1) {
        ...
        for (let i = 0; i < searcheWordApi.length; i++) {
          const labelSearch = searcheWordApi[i].label.toLowerCase().indexOf(this.keyword)
          if (labelSearch === -1) {
            const abstractSearch = searcheWordApi[i].abstract.toLowerCase().indexOf(this.keyword)
            if (abstractSearch !== -1) {
